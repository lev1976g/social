...
class mozg_template{
	...
	var $result = array ('error_yellow' => '', 'content' => '');
	...
	function load_template($tpl_name){
		...
		if($tpl_name == '' || ! file_exists($this->dir.DIRECTORY_SEPARATOR.$tpl_name)){
			msgbox('', '<div align="center">Невозможно загрузить шаблон: <strong>'.$tpl_name.'</strong></div>', 'error_red');
			return false;
		}
	...
	function sub_load_template($tpl_name){
		...
		if($tpl_name == '' || ! file_exists($this->dir.DIRECTORY_SEPARATOR.$tpl_name)){
			msgbox('', '<div align="center">Отсутствует файл шаблона: <strong>'.$tpl_name.'</strong></div>', 'error_red');
			return false;
		}
	...
	function compile($tpl){
		...
		$this->copy_template = preg_replace("#\\{lang=(.+?)\\}#ies", "\$this->load_lang('\\1')", $this->copy_template);

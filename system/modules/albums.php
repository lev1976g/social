...
if($logged){
	...
	switch($act){
		...
		case "create_page":
			NoAjaxQuery();
			$tpl->load_template('albums/albums_create.tpl');
			$tpl->compile('content');
			AjaxTpl();
			die();
		break;
					...
					if(in_array(strtolower($type), $allowed_files)){
						...
						if($image_size < $config['max_photo_size']){
							...
							if(move_uploaded_file($image_tmp, $album_dir.$image_rename.$res_type)){
								...
								//Создание маленькой копии PROFILE
								$tmb = new thumbnail($album_dir.$image_rename.$res_type);
								$tmb->size_auto('95x95');
								$tmb->jpeg_quality(100);
								$tmb->save($album_dir.'95_'.$image_rename.$res_type);
								
								//Создание маленькой копии
								$tmb = new thumbnail($album_dir.$image_rename.$res_type);
								$tmb->size_auto('220x140');
								$tmb->jpeg_quality(100);
		...
		case "edit_cover";
			...
			if($user_id AND $id){
				...
				if($sql_){
					...
					echo '<div class="error_gray" style="padding:172px 40px; margin:10px;">'.$lang['no_photo_alnumx'].'</div>';
			} else
				Hacking();
		...
		case "all_photos_box";
			...
			if($row_album['photo_num']){
				$tpl->load_template('wall/attach_photo_top.tpl');
				...
				$tpl->load_template('wall/attach_photos.tpl');
				...
				box_navigation($gcount, $row_album['photo_num'], $page, 'wall.attach_addphoto');
...
";
				echo '<div align="center" style="padding:100px 20px; padding-top:110px; text-align:center; line-height:19px; color:#869ba7; font-size:14px;">'.$lang['no_photo_alnumx'].'<br /><br /><br /><button class="green_button" id="upload">Загрузить новую фотографию</button></div>';
			}
				...
				if($sql_){
					...
					foreach($sql_ as $row_comm){
						...
						if($row_comm['user_photo'])
							$tpl->set('{ava}', $config['home_url'].'uploads/users/'.$row_comm['user_id'].'/100_'.$row_comm['user_photo']);
						else
					...
					$tpl->result['error_yellow'] .= '<div class="white_content">';
					msgbox('', $lang['no_comments'], 'error_gray');
					$tpl->result['error_yellow'] .= '</div>';
				}
			} else {
		...
		case "view":
			...
			if(!$CheckBlackList){
				...
				if($album_privacy[0] == 1 OR $album_privacy[0] == 2 AND $check_friend OR $user_info['user_id'] == $row_album['user_id']){
					...
					$tpl->result['content'] .= '<div class="white_content" style="padding:10px 0px; width:600px;">';
					$tpl->compile('error_yellow');
					...
					if($sql_photos){
						...
						$tpl->result['content'] .= '<div style="padding:0px 10px; width:580px;"><div class="error_gray">'.$lang['no_photo_alnumx'].'</div>';
						$tpl->result['content'] .= '</div>';
					}
		...
		default:
			...
			//Выводим данные о владельце альбома(ов)
			$row_owner = $db->super_query("SELECT user_search_pref, user_albums_num FROM `".PREFIX."_users` WHERE user_id = '{$uid}'");
...
if($user_info['user_id'] == $uid){
								$tpl->result['content'] .= '<div class="cover_edit_title" style="margin:-10px; padding:12px; font-size:14px; margin-bottom:10px;">У Вас <span id="badlistnum">'.$row_owner['user_albums_num'].'</span> '.gram_record($row_owner['user_albums_num'], 'albums').'</span>';
								$tpl->result['content'] .= '</div>';
							} else {
								$tpl->result['content'] .= '<div class="cover_edit_title" style="margin:-10px; padding:12px; font-size:14px; margin-bottom:10px;">У '.gramatikName($author_info[0]).' <span id="badlistnum">'.$row_owner['user_albums_num'].'</span> '.gram_record($row_owner['user_albums_num'], 'albums').'</span>';
								$tpl->result['content'] .= '</div>';
							}
						...
						if($row_owner['user_albums_num']){
							...
							msgbox('', $lang['no_albums'], 'error_gray');
						}
						$tpl->result['content'] .= '</div>';
					} else {

...
function msgbox($title, $text, $tpl_name) {
	...
	$tpl_2->compile('error_yellow');
	$tpl_2->clear();
	
	$tpl->result['error_yellow'] .= $tpl_2->result['error_yellow'];
}
...
function creat_system_cache($prefix, $cache_text){
	$filename = ROOT_DIR.'/cache/system/'.$prefix.'.php';
...
function mozg_clear_cache(){
	$fdir = opendir(ROOT_DIR.'/cache/'.$folder);
...
function mozg_clear_cache_folder($folder){
	...
	while($file = readdir($fdir))
		@unlink(ROOT_DIR.'/cache/'.$folder.'/'.$file);
}
function mozg_clear_cache_file($prefix) {
	@unlink(ROOT_DIR.'/cache/'.$prefix.'.tmp');
}
function mozg_mass_clear_cache_file($prefix){
	$arr_prefix = explode('|', $prefix);
	foreach($arr_prefix as $file)
		@unlink(ROOT_DIR.'/cache/'.$file.'.tmp');
}
function mozg_create_folder_cache($prefix){
	if(!is_dir(ROOT_DIR.'/cache/'.$prefix)){
		@mkdir(ROOT_DIR.'/cache/'.$prefix, 0777);
		@chmod(ROOT_DIR.'/cache/'.$prefix, 0777);
	}
}
...
function gram_record($num,$type){
	...
	if($type == 'friends_demands'){
		if($numres == 0){
			$gram_num_record = 'not demands';
		} elseif($numres == 1){
			$gram_num_record = 'demands';
		} elseif($numres < 5){
			$gram_num_record = 'demands';
		} elseif($numres < 21){
			$gram_num_record = 'demands';
	...
	if($type == 'user_age'){
		if($numres == 0){
			$gram_num_record = 'year';
		} elseif($numres == 1){
			$gram_num_record = 'years';
		} elseif($numres < 5){
			$gram_num_record = 'years';
		} elseif($numres < 21){
			$gram_num_record = 'year';
	...
	if($type == 'friends'){
		if($numres == 0){
			$gram_num_record = 'not friends';
		} elseif($numres == 1){
			$gram_num_record = 'friend';
		} elseif($numres < 5){
			$gram_num_record = 'friends';
		} elseif($numres < 21){
			$gram_num_record = 'friends';
		} elseif($numres == 21){
			$gram_num_record = 'friend';
...
document.getElementById('content').innerHTML = '{$lang['no_notes']}';
</script>
";
		die();
	} else
...
function AjaxTpl(){
	global $tpl, $config;
	echo str_replace('{theme}', '/templates/'.$config['temp'], $tpl->result['error_yellow'].$tpl->result['content']);
}

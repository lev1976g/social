...
if($logged){
	...
	switch($act){
		...
		case "view":
			...
			$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS gid, img, price FROM `".PREFIX."_gifts_list` ORDER by `gid` DESC", 1);
			
			$tpl->result['content'] .= '<div style="margin-bottom:10px; float:left;">';
			foreach($sql_ as $gift){
				...
				$tpl->set('{img}', $gift['img']);
				$tpl->set('{price}', $gift['price']);
				$tpl->set('{uid}', $for_user_id);
				$tpl->load_template('gifts/send_box.tpl');
				$tpl->compile('content');
			}
		...
		case "send":
			...
			if($gifts['price'] AND $user_id != $for_user_id){
				if($row['user_balance'] >= $gifts['price']){
					...
					mozg_create_cache("user_{$for_user_id}/new_gifts", 1);				
		...
		default:
			...
			if($_GET['new'] AND $user_id == $uid){
				...
				$gcount = 50;
				mozg_create_cache("user_{$user_id}/new_gifts", '');
			} else {
				$tpl->set('[no-new]', '');
				$tpl->set('[/no-new]', '');
			...
			if($owner['user_gifts']){
				...
				if($sql_where AND !$sql_)
					msgbox('', '<br /><br />Новых подарков еще нет.<br /><br /><br />', 'error_gray');
			}
	}
	$tpl->clear();
	$db->free();

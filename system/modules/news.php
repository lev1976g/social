							...
							foreach($expFriensList as $fr_id){
								...
								if($fr_info){
									if($fr_info['user_photo'])
										...
										$ava = '{theme}/images/no_avatars/no_ava_100.gif';
						...
						if($row_info){
							...
							if($row_info['attach']){
								...
								if($resLinkTitle AND $row_info['text'] == $resLinkUrl OR !$row_info['text']){
									...
									$row_info['text'] = preg_replace("/\B#(\S{1,24}+)/u", '<a href="/?go=search&query=$1&type=4">#$1</a>', $row_info['text']);
								} else {
...
HTML;
							...
							foreach($likesUseList as $likeUser){
								if($likeUser){
									...
									if($rowUser['user_photo'])
										...
										$luAva = '{theme}/images/no_avatars/no_ava_50.gif';
					...
					if($row['action_type'] == 1){
						...
						if($resLinkTitle AND $row['action_text'] == $resLinkUrl OR !$row['action_text']){
									...
									$row['action_text'] = preg_replace("/\B#(\S{1,24}+)/u", '<a href="/?go=search&query=$1&type=4">#$1</a>', $row['action_text']);
								} else {

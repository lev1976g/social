	...
	switch($act){
		...
		case "blacklist":
			...
			if($row['user_blacklist_num']){
				$tpl->result['content'] .= '<div class="cover_edit_title" style="margin:-10px; padding:12px; font-size:14px; margin-bottom:10px;">В черном списке <span id="badlistnum">'.$row['user_blacklist_num'].'</span> '.gram_record($row['user_blacklist_num'], 'peoples').'</span>';
				$tpl->result['content'] .= '</div>';
			} else {
				$tpl->result['content'] .= '<div class="cover_edit_title" style="margin:-10px; padding:12px; font-size:14px; margin-bottom:10px;">В черном списке нет пользователей</span>';
				$tpl->result['content'] .= '</div>';
			}
			$tpl->compile('error_yellow');
			if($row['user_blacklist_num'] AND $row['user_blacklist_num'] <= 100){
				$tpl->load_template('settings/bl_user.tpl');
				...
				$tpl->load_template('settings/bl_no_user.tpl');
				$tpl->compile('content');
			}
			$tpl->result['content'] .= '</div>';
		break;

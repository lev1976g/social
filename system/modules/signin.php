...
if(isset($_SESSION['user_id']) > 0){
	...
	$user_info = $db->super_query("SELECT user_id, user_email, user_group, user_balance, user_friends_demands, user_pm_num, user_lastupdate, user_photo, user_msg_type, user_delet, user_ban_date, user_search_pref FROM `".PREFIX."_users` WHERE user_id = '".$logged_user_id."'");
...
//Если есть данные о COOKIE то проверяем
	...
	$user_info = $db->super_query("SELECT user_id, user_email, user_group, user_password, user_hid, user_balance, user_friends_demands, user_pm_num, user_lastupdate, user_photo, user_msg_type, user_delet, user_ban_date, user_search_pref FROM `".PREFIX."_users` WHERE user_id = '".$cookie_user_id."'");
...
if(isset($_POST['signin']) AND !$logged){
	...
	if(!preg_match('/^(("[\w-\s]+")|([\w-]+(?:\.[\w-]+)*)|("[\w-\s]+")([\w-]+(?:\.[\w-]+)*))(@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$)|(@\[?((25[0-5]\.|2[0-4][0-9]\.|1[0-9]{2}\.|[0-9]{1,2}\.))((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){2}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\]?$)/i', $email)){
		msgbox('', $lang['not_loggin'].'<br /><a href="/restore" onClick="Page.Go(this.href); return false">Забыли пароль?</a>', 'error_red');
	} else {
		...
		if(isset($email) AND !empty($email)){
			...
			if($check_user){
				...
				msgbox('', $lang['not_loggin'].'<br /><br /><a href="/restore" onClick="Page.Go(this.href); return false">Забыли пароль?</a>', 'error_red');
		} else
			msgbox('', $lang['not_loggin'].'<br /><br /><a href="/restore" onClick="Page.Go(this.href); return false">Забыли пароль?</a>', 'error_red');
	}
}

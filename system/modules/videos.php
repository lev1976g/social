	...
	switch($act){
		...
		case "send":
			...
			if($config['video_mod_add'] == 'yes'){
				...
				if(preg_match("/src=\"http:\/\/www.youtube.com|src=\"http:\/\/youtube.com/i", 'src="'.$good_video_lnk)){
					...
					$result_video_lnk = '<object id="videoplayer3685" type="application/x-shockwave-flash" data="http://ar.local/uppod/uppod.swf" width="800" height="420"><param name="bgcolor" value="#ffffff" /><param name="allowFullScreen" value="true" /><param name="allowScriptAccess" value="always" /><param name="movie" value="http://ar.local/uppod/uppod.swf" /><param name="flashvars" value="uid=videoplayer3685&amp;&amp;st=http://ar.local/uppod/style.txt&amp;file=http://www.youtube.com/embed/'.$exp_x[0].'&amp;link=http://uppod.ru/vfvsarsvy" /></object>';
				}
				...
				if(preg_match("/value=\"http:\/\/www.rutube.ru|value=\"http:\/\/rutube.ru/i", 'value="'.$good_video_lnk)){
					...
					$result_video_lnk = '<OBJECT width="800" height="420"><PARAM name="movie" value="http://video.rutube.ru/'.$exp_frutube[1].'"></PARAM><PARAM name="wmode" value="window"></PARAM><PARAM name="allowFullScreen" value="true"></PARAM><EMBED src="http://video.rutube.ru/'.$exp_frutube[1].'" type="application/x-shockwave-flash" wmode="window" width="800" height="420" allowFullScreen="true" ></EMBED></OBJECT>';
				}
				...
				if(preg_match("/src=\"http:\/\/www.vimeo.com|src=\"http:\/\/vimeo.com/i", 'src="'.$good_video_lnk)){
					...
					$result_video_lnk = '<iframe src="http://player.vimeo.com/video/'.$exp_frutube[1].'" width="800" height="420" frameborder="0"></iframe>';
				}
				...
				if(preg_match("/src=\"http:\/\/www.smotri.com|src=\"http:\/\/smotri.com/i", 'src="'.$good_video_lnk)){
					...
					$result_video_lnk = '<object id="smotriComVideoPlayer'.$exp[1].'_1314557535.5897_7726" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="800" height="420"><param name="movie" value="http://pics.smotri.com/player.swf?file='.$exp[1].'&bufferTime=3&autoStart=false&str_lang=rus&xmlsource=http%3A%2F%2Fpics.smotri.com%2Fcskins%2Fblue%2Fskin_color.xml&xmldatasource=http%3A%2F%2Fpics.smotri.com%2Fskin_ng.xml" /><param name="allowScriptAccess" value="always" /><param name="allowFullScreen" value="true" /><param name="bgcolor" value="#ffffff" /><embed src="http://pics.smotri.com/player.swf?file='.$exp[1].'&bufferTime=3&autoStart=false&str_lang=rus&xmlsource=http%3A%2F%2Fpics.smotri.com%2Fcskins%2Fblue%2Fskin_color.xml&xmldatasource=http%3A%2F%2Fpics.smotri.com%2Fskin_ng.xml" quality="high" allowscriptaccess="always" allowfullscreen="true" wmode="opaque"  width="800" height="420" type="application/x-shockwave-flash"></embed></object>';
				}
				...
				if(in_array(strtolower($img_format), $allowed_files) && preg_match("/http:\/\//i", $photo) && $result_video_lnk){
					...
					$tmb->size_auto('192x120');
					$tmb->jpeg_quality(100);
					$tmb->save($upload_dir.'/'.$image_name.'.'.$img_format);
					
					$tmb = new thumbnail($upload_dir.'/'.$image_name.'.'.$img_format);
					$tmb->size_auto('420x257');
					$tmb->jpeg_quality(100);
					$tmb->save($upload_dir.'/420_'.$image_name.'.'.$img_format);
				}
			...
			if(preg_match("/http:\/\/www.youtube.com|http:\/\/youtube.com|http:\/\/rutube.ru|http:\/\/www.rutube.ru|http:\/\/www.vimeo.com|http:\/\/vimeo.com|http:\/\/smotri.com|http:\/\/www.smotri.com/i", $video_lnk)){
				...
				if(!$sock){
					...
					if(preg_match("/http:\/\/rutube.ru|http:\/\/www.rutube.ru/i", $video_lnk)){
						$data_rutube = iconv('koi8-u', 'utf-8', str_replace(array('[', ']'), array('&iqu;', '&iqu2;'), $html));
		...
		case "all_videos":
			...
			if($count['user_videos_num']){
				...
				box_navigation($gcount, $count['user_videos_num'], $page, 'wall.attach_addvideo');
				
				$tpl->load_template('wall/albums_editcover.tpl');
				$tpl->set('[bottom]', '');
				$tpl->set('[/bottom]', '');
				$tpl->set_block("'\\[top\\](.*?)\\[/top\\]'si","");
				$tpl->compile('content');
			} else
				echo '<div align="center" style="padding:100px 20px; padding-top:110px; text-align:center; line-height:19px; color:#869ba7; font-size:14px;">'.$lang['videos_box_none'].'</div>';
				...
				if($owner){
					...
					if($owner['user_videos_num']){
						...
						if($sql_){
							...
							msgbox('', $lang['videos_nones_videos_user'], 'error_gray');
						}
						...
						if($get_user_id == $user_id)
							...
							msgbox('', $owner['user_search_pref'].' '.$lang['videos_none'], 'error_gray');
					}
				} else
					Hacking();
			} else {
